    ...
    window.FarmGod.Library = (function () {
      ...
      const timestampFromString = function (timestr) {
        ...
        if (todayPattern !== null) {
          ...
          // No pattern matched - return current server time as fallback
          console.warn('Unable to parse timestamp:', timestr);
    ...
    window.FarmGod.Translation = (function () {
      const msg = {
        ...
        hu_HU: {
          missingFeatures:
            'A scriptnek szükségé van Prémium fiókra és Farmkezelőre!',
          options: {
            title: 'FarmGod opciók',
            warning:
              '<b>Figyelem:</b><br>- Bizonyosodj meg róla, hogy az "A" sablon az alapértelmezett és a "B" egy nagyobb mennyiségű mikró-farm<br>- Bizonyosodj meg róla, hogy a farm-filterek megfelelően vannak beállítva mielőtt használod a sctiptet',
            filterImage:
            ...
            maxloot:
              'A "B" sablont küldje abban az esetben, ha az előző támadás maximális fosztogatással járt:',
            newbarbs: 'Adj hozzá új barbár falukat:',
            button: 'Farm megtervezése',
          },
          table: {
            noFarmsPlanned:
              'A jelenlegi beállításokkal nem lehet új támadást kiküldeni.',
            origin: 'Origin',
            target: 'Célpont',
            fields: 'Távolság',
            farm: 'Farm',
    ...
    window.FarmGod.Main = (function (Library, Translation) {
          ...
          if (game_data.screen == 'am_farm') {
            $.when(buildOptions()).then((html) => {
              ...
              $('.optionButton')
                .off('click')
                .on('click', () => {
                  console.log('[FarmGod] Starting getData...');
                  ...
                  getData(
                    ...
                    console.log('[FarmGod] getData completed successfully');
                    Dialog.close();
                    ...
                    console.error('[FarmGod] Error in getData:', error);
                    UI.ErrorMessage('Error loading data. Check console for details.');
                  });
                });
      ...
      const getData = function (group, newbarbs, losses) {
        console.log('[FarmGod] getData called with:', { group, newbarbs, losses });
        let data = {
          villages: {},
          commands: {},
        ...
        let villagesProcessor = ($html) => {
          console.log('[FarmGod] villagesProcessor: Starting to process villages');
          let skipUnits = ['ram', 'catapult', 'knight', 'snob', 'militia'];
          const mobileCheck = $('#mobileHeader').length > 0;
          console.log('[FarmGod] villagesProcessor: Mobile check =', mobileCheck);
    
          if (mobileCheck) {
            let table = jQuery($html).find('.overview-container > div');
            console.log('[FarmGod] villagesProcessor: Found', table.length, 'village containers (mobile)');
            table.each((i, el) => {
              try {
                ...
                console.log(`[FarmGod] villagesProcessor [${i}]: Processing village ${name} (${coord}), ID: ${villageId}`);
                ...
                if (!coord) {
                  console.warn(`[FarmGod] villagesProcessor [${i}]: Could not extract coordinate for village ${name}`);
                  return;
                }
                ...
                console.log(`[FarmGod] villagesProcessor [${i}]: Successfully added village ${coord}`);
              } catch (e) {
                console.error(`[FarmGod] villagesProcessor [${i}]: Error processing village data:`, e);
                console.error(`[FarmGod] villagesProcessor [${i}]: Element HTML:`, jQuery(el).html());
              }
            });
          } else {
            let rows = $html
              .find('#combined_table')
              .find('.row_a, .row_b');
            console.log('[FarmGod] villagesProcessor: Found', rows.length, 'village rows (desktop)');
            ...
            rows
              ...
              .each((i, el) => {
                try {
                  ...
                  let coord = $qel.text().toCoord();
                  let name = $qel.data('text');
                  let villageId = parseInt($el.find('.quickedit-vn').first().data('id'));
                  ...
                  console.log(`[FarmGod] villagesProcessor [${i}]: Successfully added village ${coord} with ${units.length} unit types`);
                } catch (e) {
                  console.error(`[FarmGod] villagesProcessor [${i}]: Error processing village row:`, e);
                  console.error(`[FarmGod] villagesProcessor [${i}]: Row HTML:`, $(el).html());
                }
              });
          }
    
          console.log('[FarmGod] villagesProcessor: Completed. Total villages:', Object.keys(data.villages).length);
          console.log('[FarmGod] villagesProcessor: Villages data:', data.villages);
          return data;
        };
    
        let commandsProcessor = ($html) => {
          console.log('[FarmGod] commandsProcessor: Starting to process commands');
          let rows = $html
            .find('#commands_table')
            .find('.row_a, .row_ax, .row_b, .row_bx');
          console.log('[FarmGod] commandsProcessor: Found', rows.length, 'command rows');
          ...
          rows.each((i, el) => {
            try {
              ...
              if (coord) {
                let timeText = $el.find('td').eq(2).text().trim();
                let timestamp = Math.round(
                  lib.timestampFromString(timeText) / 1000
                );
                
                if (!data.commands.hasOwnProperty(coord))
                  data.commands[coord] = [];
                
                data.commands[coord].push(timestamp);
                console.log(`[FarmGod] commandsProcessor [${i}]: Added command to ${coord} at timestamp ${timestamp} (${timeText})`);
              } else {
                console.warn(`[FarmGod] commandsProcessor [${i}]: Could not extract coordinate from row`);
              }
            } catch (e) {
              console.error(`[FarmGod] commandsProcessor [${i}]: Error processing command row:`, e);
              console.error(`[FarmGod] commandsProcessor [${i}]: Row HTML:`, $(el).html());
            }
          });
    
          console.log('[FarmGod] commandsProcessor: Completed. Total command destinations:', Object.keys(data.commands).length);
          return data;
        };
    
        let farmProcessor = ($html) => {
          console.log('[FarmGod] farmProcessor: Starting to process farms');
          
          if ($.isEmptyObject(data.farms.templates)) {
            console.log('[FarmGod] farmProcessor: Processing farm templates');
            let unitSpeeds = lib.getUnitSpeeds();
            console.log('[FarmGod] farmProcessor: Unit speeds:', unitSpeeds);
    
            let templateRows = $html
              .find('form[action*="action=edit_all"]')
              .find('input[type="hidden"][name*="template"]')
              .closest('tr');
            console.log('[FarmGod] farmProcessor: Found', templateRows.length, 'template rows');
    
            templateRows.each((i, el) => {
              try {
                let $el = $(el);
                let $prevRow = $el.prev('tr');
                let farmIcon = $prevRow.find('a.farm_icon').first();
                let iconClass = farmIcon.attr('class');
                
                if (!iconClass) {
                  console.warn(`[FarmGod] farmProcessor [template ${i}]: Could not find farm icon class`);
                  return;
                }
    
                let match = iconClass.match(/farm_icon_(.*?)(\s|$)/);
                if (!match) {
                  console.warn(`[FarmGod] farmProcessor [template ${i}]: Could not extract template name from class: ${iconClass}`);
                  return;
                }
                
                let templateName = match[1];
                console.log(`[FarmGod] farmProcessor [template ${i}]: Processing template "${templateName}"`);
                ...
                let speed = Math.max(
                  ...$el
                    ...
                    .map((index, element) => {
                      let unitCount = $(element).val().toNumber();
                      if (unitCount > 0) {
                ...
                console.log(`[FarmGod] farmProcessor [template ${i}]: Added template "${templateName}":`, data.farms.templates[templateName]);
              } catch (e) {
                console.error(`[FarmGod] farmProcessor [template ${i}]: Error processing template:`, e);
                console.error(`[FarmGod] farmProcessor [template ${i}]: Row HTML:`, $(el).html());
              }
            });
            
            console.log('[FarmGod] farmProcessor: Templates completed:', data.farms.templates);
          }
    
          let farmRows = $html
            .find('#plunder_list')
            .find('tr[id^="village_"]');
          console.log('[FarmGod] farmProcessor: Found', farmRows.length, 'farm rows');
    
          farmRows.each((i, el) => {
            try {
              let $el = $(el);
              let reportLink = $el.find('a[href*="screen=report&mode=all&view="]').first();
              let coord = reportLink.text().toCoord();
              
              if (!coord) {
                console.warn(`[FarmGod] farmProcessor [farm ${i}]: Could not extract coordinate from report link`);
                return;
              }
    
              let villageId = $el.attr('id').split('_')[1].toNumber();
              let dotImg = $el.find('img[src*="graphic/dots/"]');
              let colorMatch = dotImg.attr('src')?.match(/dots\/(green|yellow|red|blue|red_blue)/);
              let color = colorMatch ? colorMatch[1] : null;
              let maxLoot = $el.find('img[src*="max_loot/1"]').length > 0;
              ...
              console.log(`[FarmGod] farmProcessor [farm ${i}]: Added farm ${coord}:`, data.farms.farms[coord]);
            } catch (e) {
              console.error(`[FarmGod] farmProcessor [farm ${i}]: Error processing farm row:`, e);
              console.error(`[FarmGod] farmProcessor [farm ${i}]: Row HTML:`, $(el).html());
            }
          });
    
          console.log('[FarmGod] farmProcessor: Completed. Total farms:', Object.keys(data.farms.farms).length);
          return data;
        };
    
        let findNewbarbs = () => {
          if (newbarbs) {
            console.log('[FarmGod] findNewbarbs: Starting to fetch new barbs from /map/village.txt');
            return twLib.get('/map/village.txt').then((allVillages) => {
              let lines = allVillages.match(/[^\r\n]+/g);
              console.log('[FarmGod] findNewbarbs: Found', lines.length, 'total villages in village.txt');
              let barbCount = 0;
              
              lines.forEach((villageData, index) => {
                try {
                  ...
                  if (
                    ...
                    barbCount++;
                    if (barbCount <= 10) {
                      console.log(`[FarmGod] findNewbarbs: Added new barb at ${coord}, ID: ${id}`);
                    }
                  }
                } catch (e) {
                  console.error(`[FarmGod] findNewbarbs [line ${index}]: Error parsing village data:`, e, villageData);
                }
              });
    
              console.log('[FarmGod] findNewbarbs: Completed. Added', barbCount, 'new barbarean villages');
              return data;
            }).catch((error) => {
              console.error('[FarmGod] findNewbarbs: Error fetching village.txt:', error);
              return data;
            });
          } else {
            console.log('[FarmGod] findNewbarbs: Skipped (newbarbs = false)');
            return data;
          }
        };
    
        let filterFarms = () => {
          console.log('[FarmGod] filterFarms: Starting farm filtering. Initial farm count:', Object.keys(data.farms.farms).length);
          let initialCount = Object.keys(data.farms.farms).length;
          ...
          data.farms.farms = Object.fromEntries(
            Object.entries(data.farms.farms).filter(([key, val]) => {
              ...
              if (!shouldKeep) {
                console.log(`[FarmGod] filterFarms: Filtering out farm ${key} with color ${val.color}`);
              }
          ...
          let finalCount = Object.keys(data.farms.farms).length;
          console.log(`[FarmGod] filterFarms: Completed. Filtered out ${initialCount - finalCount} farms. Remaining: ${finalCount}`);
          return data;
        };
    
        console.log('[FarmGod] getData: Starting Promise.all for all processors');
        return Promise.all([
          lib.processAllPages(
            ...
            villagesProcessor
          ).then(() => console.log('[FarmGod] getData: Villages processing completed')),
          lib.processAllPages(
            TribalWars.buildURL('GET', 'overview_villages', {
              mode: 'commands',
              type: 'attack',
            ...
            commandsProcessor
          ).then(() => console.log('[FarmGod] getData: Commands processing completed')),
          lib.processAllPages(
            TribalWars.buildURL('GET', 'am_farm'),
            farmProcessor
          ).then(() => console.log('[FarmGod] getData: Farm processing completed')),
          findNewbarbs(),
        ])
          .then(() => {
            console.log('[FarmGod] getData: All parallel processes completed');
            return filterFarms();
          })
          .then(() => {
            console.log('[FarmGod] getData: Filter completed. Returning final data');
            console.log('[FarmGod] getData: Final data summary:', {
              villages: Object.keys(data.villages).length,
              commands: Object.keys(data.commands).length,
              templates: Object.keys(data.farms.templates).length,
              farms: Object.keys(data.farms.farms).length,
            });
            return data;
          })
          .catch((error) => {
            console.error('[FarmGod] getData: Critical error in promise chain:', error);
            throw error;
          });
      };
      ...
      const createPlanning = function (
        ...
        console.log('[FarmGod] createPlanning: Starting with options:', { optionDistance, optionTime, optionMaxloot });
        let plan = { counter: 0, farms: {} };
        let serverTime = Math.round(lib.getCurrentServerTime() / 1000);
        console.log('[FarmGod] createPlanning: Server time:', serverTime);
        ...
        for (let prop in data.villages) {
          ...
          orderedFarms.forEach((el) => {
            ...
            if (!template) {
              console.warn(`[FarmGod] createPlanning: Template "${template_name}" not found for farm ${el.coord}`);
              return;
            }
        ...
        console.log('[FarmGod] createPlanning: Completed. Total farms planned:', plan.counter);
        return plan;
      };
